...
| **E4.5 — Cross-screen design consistency** | Standardize the Control Center shell and every sub-tab against the shared Lumière Ledger page system: page headers, typography, content width, cards, tab controls, buttons, form fields, empty/loading/error states, spacing, and responsive behavior. Validate every option on desktop and installed-PWA widths. Replace isolated one-off styling with reusable primitives where doing so is low-risk; preserve all existing workflows. After the pattern is verified, add the general consistency requirement to the Global AI Rules so future screens must match their product's established design system. | Added from authenticated review 2026-09-09; not started. |
