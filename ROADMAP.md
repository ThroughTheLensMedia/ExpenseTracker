...
| **`profiles` table doesn't exist — `cron.js` monthly-report is fatally broken** | Confirmed via Vercel runtime errors 2026-07-01: `/cron/monthly-report` throws `PGRST205: Could not find the table 'public.profiles'` and never sends. `admin.js` lines 52/397 hit the same missing table but degrade silently via `.catch(() => ({ data: [] }))` — masks the same root problem elsewhere. Only `user_roles` exists in public schema, and it has no email/display_name columns. Needs a real decision: recreate `profiles` or repoint all 4 call sites at the correct source of user email/display_name. |
...
| **`user_daily_activity` duplicate-key error** | Confirmed via Vercel runtime errors: `useActivityPulse` write hits `23505 duplicate key` on `(user_id, activity_date)` — happened 2026-06-30 and 2026-07-01 on Joshua's own account. Likely an `INSERT` that should be an `UPSERT ... ON CONFLICT`. Non-fatal but recurring since at least March 2026. |
