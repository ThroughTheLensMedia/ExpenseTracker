...
**Version:** v7.15.3 | **Last reviewed:** 2026-07-06  
...
| **B3 — Weekly digest email** | ✅ built and live (v7.15.3) — `GET /api/cron/weekly-report`. **Incident 2026-07-06 (resolved): sent 100+ real emails to Joshua over several hours.** Root cause was NOT the `force`/de-dupe flag logic (fixed in v7.15.1, insufficient alone) — the actual cause was `settings` missing 6 columns this route depends on, so the de-dupe `SELECT` errored silently and the guard never engaged. Migration `add_missing_settings_columns_v7_15_2` applied directly to production; both settings queries now fail closed on any DB error (v7.15.3) instead of silently proceeding. One real send + de-dupe-on-second-ping verification pending immediately post-deploy. Admin-only gate (`ADMIN_UUID` filter, v7.13.2) remains in place underneath. |
