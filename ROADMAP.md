...
| **E2 — Dashboard decisions and Assistant trust** | Prioritize next actions, clarify metrics, and reconcile Assistant capability guidance with the tools it can actually use. | ✅ Protected preview verified 2026-09-09 in Business and Personal modes at desktop and 390×844 mobile; priority actions, mode-aware Assistant guidance, non-expiring account status, responsive layout, API health, and browser-console checks pass. Production release remains unapproved. |
| **E3 — Core financial workflows** | Improve transaction review, saved views, account metric clarity, tax guidance, and mileage fallback messaging. | Reconciled onto current main and deployed to protected preview 2026-09-09; 16 focused tests and production build pass. Transactions, Accounts, Tax, and Mileage were exercised in authenticated QA; latest unique-preview re-login and responsive-width verification remain open. Account restored to Personal; no production release. |
| **E4.1 — Client-to-invoice workflow** | Strengthen the lead-to-client-to-invoice flow with direct qualified-lead and client handoffs, exact invoice links, duplicate-client prevention, and clearer workflow states. | ✅ Protected preview approved 2026-09-09; 12 focused tests, targeted lint, production build, and authenticated user review pass. Production release remains unapproved. |
| **E4.2 — Document workflow** | Make document upload, organization, retrieval, and failure recovery easier to understand without changing storage or AI extraction architecture. | ✅ Protected preview approved 2026-09-09; search, type filtering, processing feedback, retrieval/edit actions, and recoverable states verified without requiring a Gemini key for document access. Production release remains unapproved. |
| **E4.5A — Shared foundation and Control Center shell** | Establish the shared responsive page foundation, then align the Control Center header, navigation, content frame, and accessible tab states without changing workflows. | ✅ Protected preview approved 2026-09-10; production build and authenticated desktop shell/tab checks pass. Production release remains unapproved. |
| **E4.5B — User-facing Control Center sections** | Apply the approved pattern to Experience, AI Intelligence, Automation, Categories, Dashboard, Documents, Help Center, Integrations, and Profile using low-risk shared styles. | ✅ Protected preview approved 2026-09-10; all nine user-facing sections passed authenticated desktop review, production build, browser-console checks, and runtime verification. Intake-key routing compatibility fix verified with `/api/intake-keys` returning 200. Production release remains unapproved. |
...
| **E5 — Pilot and controlled release** | Run responsive and end-to-end verification, review the diff, update both changelogs and version files, then request separate production approval. | In progress 2026-09-10; tests (12/12), production build, and `git diff --check` pass on v7.29.0. Diff reviewed — no auth/billing/Plaid/migration changes. A pre-existing dashboard readability issue (Top Spend / Recurring Vendors vendor column illegible on narrow screens) was found during acceptance and fixed in-scope. Pushed to protected preview; authenticated desktop and 390×844 walkthrough by the owner is in progress. Not yet marked complete — production release remains unapproved. |
...
| Recurring Vendors: distinguish subscriptions from repeat merchants | Flagged during E5 acceptance review (2026-09-10) — the dashboard's Operational Intelligence "Recurring Vendors" table currently lists any vendor with `count >= 3` charges in the trailing 12 months (`api/routes/metrics.js`), with zero amount-consistency or interval-regularity check. This puts genuinely recurring merchants that aren't subscriptions (e.g. a frequently-visited restaurant, a gas station, a quarterly-billed landscaper) on the same list as real subscriptions, distinguished only by a "SUB" badge that itself relies on a tiny hardcoded 11-vendor substring list (`KNOWN_SUBSCRIPTION_VENDORS` in `api/utils/spendCategories.js`: adobe, netflix, hulu, spotify, apple, google workspace, squarespace, chatgpt, openai, amazon web services, aws) plus the user-set `is_subscription` flag. Rocket Money's publicly known approach adds amount-consistency and interval-regularity checks (charges land on a consistent cadence, not just "happened 3 times") plus a much larger merchant-category database, rather than treating raw repetition as the signal. Future improvement: add an interval-regularity check (charges cluster around a consistent day-count gap) and amount-consistency check (charges within a tolerance band of each other) before a vendor is treated as "recurring," and/or split the table into "Recurring Charges" (repetition-based) vs. "Confirmed Subscriptions" (amount+interval+category-based) so the two concepts aren't conflated in one list. Not built — needs design discussion on thresholds before implementation. |
...
- **Non-issues confirmed during E5 protected-preview review (2026-09-10) — do not re-chase:** (1) `connect.facebook.net/en_US/fbevents.js` `ERR_BLOCKED_BY_CLIENT` in the browser console — caused by the viewer's own ad blocker/browser extension blocking the Meta Pixel tracking script client-side; not reproducible without a blocking extension and has no effect on app functionality. (2) `vercel.com/sso-api` CORS errors on `manifest.json` — Vercel's SSO Deployment Protection intercepting the PWA manifest fetch on **preview deployments only**; production has no SSO gate and does not exhibit this. Neither is fixable or worth fixing in application code.
- **✅ `apple-mobile-web-app-capable` deprecated meta warning (v7.29.0)** — added the standard `mobile-web-app-capable` meta tag alongside the existing Apple-specific one in `web-react/index.html`, per the deprecation warning Chrome DevTools raises. No behavior change, resolves the console warning.
