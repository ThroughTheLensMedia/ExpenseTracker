...
1. ✅ **v7.15.0 — Encrypt `settings.gemini_api_key` at rest.** Shipped: `decryptOrPlain()` helper in `cryptoUtil.js`, encrypt-on-save/decrypt-on-read in `settings.js` and all 6 downstream read sites (`brain.js` ×2, `receipts.js`, `documents.js`, `import.js`, `emailInbound.js`), one-time migration script `api/scripts/encrypt-existing-gemini-keys.js` (idempotent, `--dry-run` supported). **v7.15.4:** `ENCRYPTION_KEY` is Sensitive in Vercel and can't be exported locally, so added a temporary admin route (`POST /admin/encrypt-gemini-keys`, `?dry_run=1` supported) to run the same migration server-side — same precedent as the v7.10.17 Plaid webhook backfill. Route to be removed once the live run confirms 0 failures.
2. Persistent conversation memory (rolling window, not full transcript storage) — in progress (Joshua confirmed building all three ahead of usage volume, 2026-07-06).
