...
**Version:** v7.15.2 | **Last reviewed:** 2026-07-06  
...
| **B3 — Weekly digest email** | ✅ built, ⚠️ **paused for verification (v7.15.2)** — `GET /api/cron/weekly-report` returns early with no sends (`?preview=1` still works for dry-run testing). **Incident 2026-07-06: sent 100+ real emails to Joshua over several hours.** Root cause was NOT the `force`/de-dupe flag logic (fixed in v7.15.1, insufficient alone) — the actual cause was `settings` missing the 6 columns this route depends on (`estimated_tax_rate`, `last_weekly_digest_sent_at`, etc.), so the de-dupe `SELECT` errored silently and the guard never engaged regardless of code logic. Migration `add_missing_settings_columns_v7_15_2` applied directly to production. Sending stays paused until Joshua confirms one real send correctly de-dupes on a second ping — see `cron.js` to re-enable. Admin-only gate (`ADMIN_UUID` filter, v7.13.2) remains in place underneath regardless. |
...
- **🔴 Weekly digest resend incident (2026-07-06) — root cause + lesson** — Phase B (v7.13.0) shipped code referencing 6 `settings` columns that were never actually migrated into the database (`estimated_tax_rate`, `weekly_digest_optout`, `reengagement_email_optout`, `last_weekly_digest_sent_at`, `last_reengagement_sent_at`, `money_story_optout`). The resulting `SELECT`/`upsert` calls in `cron.js` errored on every call — silently, because neither call site checks the returned `error` — so the de-dupe guard's settings map was always empty and 100+ real emails went out over several hours before being caught. Same bug class as the existing "Silent Supabase failure pattern" (`{ error }` never throws) already tracked from prior security reviews. **Fixed:** migration `add_missing_settings_columns_v7_15_2` applied directly to production. **Still open:** add explicit error-checking/logging to the `settings` select/upsert calls in `cron.js` (both weekly-report and reengagement-report) so a future schema drift fails loudly instead of silently disabling a safety guard. **Process lesson:** any code referencing a new `settings` column needs its migration verified against the live schema (`information_schema.columns`), not assumed from the generic upsert accepting arbitrary JSON keys at the API layer.
