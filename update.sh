...
echo "---- Quick Update Expense Tracker ----"
...
echo "Step 3. Syncing Nginx Config..."
(cd web && tar cz nginx.conf Dockerfile) | ssh "${NAS_USER}@${NAS_HOST}" "mkdir -p \"${NAS_PATH}/web\" && tar xz -C \"${NAS_PATH}/web\" 2>/dev/null" || exit 1

echo "Step 4. Finalizing stack on NAS (Password required for Docker)..."
ssh -t "${NAS_USER}@${NAS_HOST}" "cd \"${NAS_PATH}\" && sudo chmod -R 777 data && sudo /usr/local/bin/docker compose restart expense_api expense_web" || (
  echo "!! Update Failed. Attempting to fetch logs..."
