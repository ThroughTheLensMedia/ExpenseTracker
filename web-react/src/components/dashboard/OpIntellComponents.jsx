...
export const TopOffendersSnapshot = ({ offenders }) => {
    ...
    return (
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '15px', alignItems: 'center' }}>
            ...
            {offenders.map(o => (
                <div key={o.id} style={{ display: 'flex', alignItems: 'baseline', gap: '6px', background: 'rgba(255,255,255,0.03)', padding: '4px 12px', borderRadius: '12px', maxWidth: '100%' }}>
                    <span
                        title={o.vendor}
                        style={{ fontSize: '12px', fontWeight: 800, textTransform: 'capitalize', maxWidth: 'min(220px, 55vw)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}
                    >
                        {o.vendor}
                    </span>
                    <span style={{ fontSize: '12px', fontWeight: 700, color: 'rgba(255,255,255,0.5)' }}>—</span>
                    <span className="money" style={{ fontSize: '12px', fontWeight: 900, color: '#38bdf8', whiteSpace: 'nowrap' }}>${fmtCents(o.monthly_cost)}/mo</span>
                </div>
            ))}
        </div>
    );
                ...
                <tbody>
                    ...
                    {rows.map((row, idx) => (
                        <tr onClick={() => onRowClick && onRowClick(row.vendor)} key={idx} style={{ borderBottom: '1px solid rgba(255,255,255,0.02)', cursor: 'pointer' }} className="table-row-hover">
                            <td style={{ padding: '12px 15px', fontWeight: 800, textTransform: 'capitalize', position: 'sticky', left: 0, background: 'var(--bg)', maxWidth: 160, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: mergingVendor === row.vendor ? 'normal' : 'nowrap' }} title={mergingVendor === row.vendor ? undefined : row.vendor}>
