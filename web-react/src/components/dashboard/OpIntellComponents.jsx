    ...
    return (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '15px', marginBottom: '20px' }}>
            ...
            <div style={{ padding: '15px', background: 'rgba(56, 189, 248, 0.05)', border: '1px solid rgba(56, 189, 248, 0.2)', borderRadius: '12px', display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center' }}>
                ...
                <div style={{ fontWeight: 900, color: '#38bdf8', fontSize: '24px', marginTop: '4px' }}>${(summary.monthly_total / 100).toLocaleString('en-US', {maximumFractionDigits: 0})}<span style={{ fontSize: '12px' }}>/mo</span></div>
            </div>
            ...
            <div style={{ padding: '15px', background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.05)', borderRadius: '12px', display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center' }}>
                ...
                <div style={{ fontWeight: 900, color: 'white', fontSize: '24px', marginTop: '4px' }}>${(summary.annual_total / 100).toLocaleString('en-US', {maximumFractionDigits: 0})}<span style={{ fontSize: '12px' }}>/yr</span></div>
            </div>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '15px', alignItems: 'center' }}>
            ...
            {offenders.map(o => (
                <div key={o.id} style={{ display: 'flex', alignItems: 'baseline', gap: '6px', background: 'rgba(255,255,255,0.03)', padding: '4px 12px', borderRadius: '12px' }}>
                    ...
                    <span style={{ fontSize: '12px', fontWeight: 900, color: '#38bdf8' }}>${(o.monthly_cost / 100).toLocaleString('en-US', {maximumFractionDigits: 0})}/mo</span>
                </div>
            ))}
        </div>
    );
                ...
                <tbody>
                    ...
                    {rows.map((row, idx) => (
                        <tr onClick={() => onRowClick && onRowClick(row.vendor)} key={idx} style={{ borderBottom: '1px solid rgba(255,255,255,0.02)', cursor: 'pointer' }} className="table-row-hover">
                            ...
                            <td style={{ padding: '12px 15px', textAlign: 'right' }}>${(row.monthly_cost / 100).toLocaleString('en-US', {maximumFractionDigits: 0})}/mo</td>
                            <td style={{ padding: '12px 15px', textAlign: 'right', color: 'rgba(255,255,255,0.6)' }}>${(row.annual_cost / 100).toLocaleString('en-US', {maximumFractionDigits: 0})}/yr</td>
