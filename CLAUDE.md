...
- **Exempt users:** `PLAID_BILLING_EXEMPT` — single source of truth is `api/constants.js`; frontend mirror is `web-react/src/constants/billing.js`. Joshua, Michelle Gornichec (`fcb92809-70f1-4ae0-b39c-e317378a01a7`), and Jason Deuermeyer (`1fd43a56-7b9d-4928-9b01-236e29753023`) are owner-covered; everyone else pays.
...
| `api/constants.js` | Single source of truth for backend identity/billing constants and the allowed `EXPERIENCE_MODES` values. Import from here — do not hardcode a new copy. |
...
| File | Purpose |
|------|---------|
| `web-react/src/components/AuthContext.jsx` | Global auth + session persistence. Exports `supabase` client. Local mock-user bypass requires both `VITE_ENABLE_DEV_BYPASS=true` and the legacy local-storage flag. |
...
| `web-react/src/pages/PersonalDashboard.jsx` | Personal-mode overview — monthly income, spending, cash flow, categories, and recurring bills from the shared metrics endpoint. |
...
| `web-react/src/components/control-center/DashboardTab.jsx` | Business dashboard customization — role cards + widget toggles. Saves to `settings.dashboard_config`. |
| `web-react/src/components/control-center/ExperienceTab.jsx` | Reversible Personal/Business experience selector; changes presentation only and preserves all data. |
...
| `web-react/src/constants/experienceModes.js` | Personal/Business experience constants, safe Business-default resolver, and business-only route inventory. App shell behavior must derive from this module. |
| `web-react/src/pages/Login.jsx` | Shared, mode-neutral auth screen — email/password, Google OAuth, invite-code signup, password reset, and account requests. Cloudflare Turnstile protects signup. |
...
**Database — SQL Schemas** — `supabase_schema.sql` (expenses, classification_rules, mileage_logs, mileage_rates), `supabase_schema_rls.sql` (RLS policies), `supabase_schema_settings.sql` (settings incl. `dashboard_config`), `supabase_schema_activity.sql`, `supabase_schema_leads.sql`, `supabase_schema_plaid.sql`, `supabase_schema_intake_keys.sql`, `api/migrations/009_user_categories.sql` through `api/migrations/020_add_experience_mode_to_settings.sql` (see `api/migrations/` for the full incremental list — `011_brain_messages.sql` is persistent AI Brain conversation history, RLS-scoped; `017_add_notes_to_documents.sql` adds `user_documents.notes`; `018_add_reviewed_to_vendor_settings.sql` and `019_add_vendor_review_note.sql` are covered in the Vendor review flags data pattern below; `020_add_experience_mode_to_settings.sql` adds the Business-default account experience selector), `supabase_fix_admin_rls.sql`.
...
| Recurring vendor cadence | `expenses.billing_cycle` (nullable: `monthly`/`quarterly`/`annual`, added v7.20.1) — set via Transaction Drawer when "Recurring" is checked (v7.21.0). `api/utils/recurringVendors.js`'s `deriveCadenceDays()` uses it if set, else derives cadence from real charge-date gaps (2+ occurrences), else falls back to a naive total/count average flagged `cadenceLabel: 'Unconfirmed'`. Shared by `metrics.js` (dashboard) and `cron.js` (weekly digest forecast) so both agree. **v7.27.5:** the explicit tag is now sanity-checked against the real average gap between charges — if actual charges are coming in much faster than the tag implies (`detectedCadenceDays < explicitDays * 0.5`), that signals the vendor bucket actually holds more than one distinct billed item under the same name (e.g. two domain renewals both posting as "Hover"), and the derived real cadence is used instead (`source: 'detected-override'`) so the monthly-equivalent isn't silently understated. `cadenceLabel` only echoes the tag's label when `source === 'explicit'`. **v7.27.5:** `metrics.js`'s `vendorActivity` tracking (feeds Operational Intelligence only — not YTD/monthly-performance figures elsewhere in the same endpoint) also switched from a calendar-year gate to a rolling trailing-365-days window, so pricing drift isn't diluted by years-old charges and nothing resets near-empty every January. |
| Vendor review flags | Two independent, unrelated flags — don't conflate them. (1) `vendor_settings.is_ignored`/`is_reviewed` (booleans, user-scoped by `vendor` text) — dismiss the Operational Intelligence REVIEW badge (`flags.review` in `metrics.js`, fires when `count < 6 && avgCostPerOccurrence > $20`) via CONFIRM/UNDO, or hide a vendor from recurring rollups entirely via IGNORE/RESTORE. `POST /vendors/settings` (v7.27.4) accepts either field independently and merges with the existing row first — never send one without fetching the other or it clobbers back to `false`. (2) `expenses.vendor_review_note` (nullable text, added v7.27.5, `api/migrations/019_add_vendor_review_note.sql`) — flags one specific CSV-imported *transaction* whose vendor string closely resembles an existing recurring vendor (2+ charges) without exactly matching it (bank-descriptor variant, e.g. address baked into the string). Set by `api/utils/vendorVariant.js`'s `findVendorVariantMatch()` during CSV import only (not Plaid sync yet). Shown as a blue 🔗 badge in `Transactions.jsx`, auto-cleared by `PATCH /expenses/:id` whenever `vendor` is explicitly changed (single or `bulk-vendor`). |
...
| Bulk edit — overwrite vs. append | `PATCH /expenses/bulk-source`, `bulk-vendor` (v7.27.0), and `bulk-category` (v7.26.0) all do a blind overwrite — correct, since account/vendor/category are single-value fields being reassigned to one new value across the batch. `PATCH /expenses/bulk-notes` (v7.26.0) instead APPENDS a dated entry (`[YYYY-MM-DD] text`) to each row's existing notes — different selected rows likely already hold different notes text, so an overwrite would silently destroy it. General rule: a bulk field update should overwrite only when the field represents one current value being replaced; if the field is free-text that varies per row and accumulates meaning over time (notes, comments, history), append instead. `bulk-vendor` rewrites the actual `vendor` column (permanent, affects search/filter/dupe-scan everywhere) — distinct from `vendor_aliases` (used only by `metrics.js`'s Dashboard Operational Intelligence rollup), which maps a display name at aggregation time without touching the underlying transaction rows. Pick `bulk-vendor` to actually fix a name typo/variant; pick `vendor_aliases` to normalize a display name while preserving the original imported bank-statement string. |
| Plaid billing exempt | `PLAID_BILLING_EXEMPT` from `api/constants.js`: Joshua, Michelle Gornichec, and Jason Deuermeyer. Frontend mirror: `web-react/src/constants/billing.js`. |
...
| Experience mode | `settings.experience_mode` is constrained to `business` or `personal` by `api/migrations/020_add_experience_mode_to_settings.sql`. Missing, malformed, and all pre-existing values resolve to `business`. It controls presentation/navigation only; it is not a subscription/security boundary and must never delete or transform user data. Backend values come from `api/constants.js`; frontend shell behavior comes from `web-react/src/constants/experienceModes.js`. Local review may set `VITE_PREVIEW_EXPERIENCE_MODE` and matching API-only `PREVIEW_EXPERIENCE_MODE`; both overrides are development-only and inert in production. |
