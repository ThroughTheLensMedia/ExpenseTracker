...
**Database — SQL Schemas** — `supabase_schema.sql` (expenses, classification_rules, mileage_logs, mileage_rates), `supabase_schema_rls.sql` (RLS policies), `supabase_schema_settings.sql` (settings incl. `dashboard_config`), `supabase_schema_activity.sql`, `supabase_schema_leads.sql`, `supabase_schema_plaid.sql`, `supabase_schema_intake_keys.sql`, `api/migrations/009_user_categories.sql` through `api/migrations/019_add_vendor_review_note.sql` (see `api/migrations/` for the full incremental list — `011_brain_messages.sql` is persistent AI Brain conversation history, RLS-scoped; `017_add_notes_to_documents.sql` adds `user_documents.notes`; `018_add_reviewed_to_vendor_settings.sql` and `019_add_vendor_review_note.sql` are covered in the Vendor review flags data pattern below), `supabase_fix_admin_rls.sql`.
